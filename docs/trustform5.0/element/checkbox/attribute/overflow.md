...
_visible : 모든 자식 아이템이 화면에 표시될 수 있도록 checkbox 영역을 확장하거나 축소한다._ hidden : 아이템들이 checkbox의 영역을 벗어나면 해당 영역은 화면에 보이지 않도록 한다. \*scroll : 아이템들이 checkbox의 영역을 벗어나면 스크롤바를 표시한다.
